...
#            LOGGER.info('HTTP exception: %s', e)
#            #raise SystemExit(e)

        msg = byte_to_json(response.content)
        LOGGER.info('Upgrade Class response: %s', msg)
        LOGGER.info('HTTP Response ID: %d', response.status_code)
        if response.status_code == 202:
            response_json = byte_to_json(response.content)
            if not 'request_id' in response_json.keys():
...
#    LOGGER.info(lcm_json)
    ...
    LOGGER.info('Upgrade Task ID: %s', taskid)
    if taskid == 'error':
