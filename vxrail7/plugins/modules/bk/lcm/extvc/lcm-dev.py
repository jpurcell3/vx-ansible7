...
class ExpansionUrls():
    ...
    all_requests_url_tpl = 'https://{}/rest/vxm/v1/requests/'
#    lcm_url_tpl = 'https://{}/rest/vxm/v1/lcm/upgrade'
    ...
    def all_requests(self):
        return ExpansionUrls.all_requests_url_tpl.format(self.vxm_ip)
...
class VxRail():
    def __init__(self):
        ...
        self.tgt_esx = module.params.get('tgt_esx')
...
#        lcm_json['upgrade_sequence'] = {'preferred_fault_domain_first': 'true'}
        ...
        migration['target_vcsa_host'] = {'name': self.tgt_esx, 'user' : {'username' : self.vcroot, 'password' : self.vcrootpasswd}}
    ...
    def upgrade_cluster(self, lcm_json):
        ...
        try:
            ...
            LOGGER.error('%s LCM Upgrade resulted in an exception %s', err, self.vxm_ip)
            return 'error'

        LOGGER.info('LCM Upgrade response: %s', response)
    ...
    def get_error(self):
        ...
        session = requests.Session()
        try:
            response = session.get(url=self.expansion_urls.all_requests(),
                                   verify=False,
                                   auth=self.auth
                                   )
        except Exception as http_err:
        ...
        if response:
            response_json = byte_to_json(response.content)
            if response_json:
                last_err = response_json[0].get('error')
#                LOGGER.error('VxRail Error Message: %s', last_err)
                LOGGER.error('VxRail Error Message: %s', response_json)
        return last_err
...
def main():
    ...
    if VxRail().get_vc_mode() == 'EMBEDDED':
        LOGGER.info("vCenter mode is Embedded")
        lcm_json = VxRail().intvc_json()
    else:
        LOGGER.info("vCenter mode is External")
        lcm_json = VxRail().extvc_json()
    LOGGER.info('lcm_json')
    taskid = VxRail().upgrade_cluster(lcm_json)
    LOGGER.info(taskid)
    if taskid == 'error':
        t_err = VxRail().get_error()
        LOGGER.error('Upgrade failed due to %s', t_err)
