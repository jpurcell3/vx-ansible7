...
class vxrail():
    ...
    def get_task_status(self, task_id):
        ...
        init_status = []
        try:
            response = session.get(url=self.expansion_urls.get_url_task_status(task_id),
                                   verify=False,
                                   auth=None,
                                   )
        ...
        init_status = response_json.get('state')
        LOGGER.info('Task Status: %s', init_status)
        if init_status == 'COMPLETED':
            LOGGER.info("The task has completed")
        elif init_status == 'FAILED':
            ...
            LOGGER.info('Inner track_task method: %s', init_status)
            time.sleep(10)
...
def main():
    ...
    if task_id:
        ...
        while init_status not in ('COMPLETED', 'FAILED'):
            LOGGER.info("cluster_initialization: sleeping 60 seconds...")
            time.sleep(60)
            init_status = vxrail().get_task_status(task_id)
            LOGGER.info('cluster_init: track_initialization status: %s', init_status)
    else:
        init_status = validation_status
