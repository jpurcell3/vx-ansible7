...
ANSIBLE_METADATA = {
    ...
    'status': ['preview'],
    'supported_by': 'jeff.purcell@dell.com'
}
...
author: jeff.purcell@dell.com'
supported_by: VxRail Ansible Team (@jpurcell3)
...
import ssl, atexit, requests, uuid, time
...
def run_module():
    module = AnsibleModule(
        argument_spec = dict(
            ...
            portgroup_name  = dict(required=False, default='Management'),
            dvs_name       = dict(required=False, default=None)
        ),
    ...
    if (len(dv_switches) == 0): 
        ...
        if dvs_name == None:
            ...
            for vds in dv_switches:
                if vds.name == dvs_name:
                    dv_switch = vds
                    break
                
                if dv_switch == None:
                    module.fail_json(msg=('No switch found with name %s ' %  dvs_name))
    else: 
